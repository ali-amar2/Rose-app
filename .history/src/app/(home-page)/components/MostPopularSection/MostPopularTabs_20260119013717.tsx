    ...
    return (
        <Tabs value={selected} onValueChange={(val) => val && onChange(val)} >
            <TabsList className="bg-white">
                ...
                {occasions.map((occ) => (
                    <TabsTrigger key={occ._id} value={occ._id} className="text-lg font-bold data-[state=active]:bg-white data-[state=active]:text-maroon-600 data-[state=active]:shadow-none">
                        {occ.name}
                    </TabsTrigger>
                ))}
            </TabsList>
